    ...
    useEffect(() => {
        const fetchWishlistAndRecipes = async () => {
            try {
                ...
                const detailedRecipesPromises = wishlist.map(async (wishlistItem, index) => {
                    const recipeDetails = await getRecipeById(wishlistItem.RecipeID);
                    return {
                        segmentText: recipeDetails.recipeName,
                        segColor: ['#f49097', '#dfb2f4 ', '#f5e960 ', '#f2f5ff ', '#55d6c2'][index % 5], // Cycle through colors
                    };
                });
                ...
                console.error("Error fetching recipes by userID or recipe details:", error);
            }
        };
